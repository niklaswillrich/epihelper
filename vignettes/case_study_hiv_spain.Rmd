...
_This is the R version of an original Stata-based case study prepared by Alicia Barrasa and Ioannis Karagiannis, based on the original case study developed in 2009 by Alicia Barrasa, Costas Danis, Manuel Dehnert, Johannes Dreesman, Matthias an der Heiden, Harold Noël-Placidoux, Ioannis Karagiannis and David Prieto_
...
# require(epihelper) # various helper functions for epidemiologists
...
```{r}
save.image("HIVexample2015poisson.Rdata")
```



## Show table of incidence rates along exposure idu and exposure time denom
```{r}
ir(hiv.data.sum$seroco, 
   hiv.data.sum$idu,
   hiv.data.sum$denom, output="rmarkdown")
```
